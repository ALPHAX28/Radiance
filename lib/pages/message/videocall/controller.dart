...
class VideoCallController extends GetxController {
  ...
  Future<void> _initEngine() async {
    await player.setAsset("assets/marimba-for-smartphone-loop-151931.mp3");
  ...
  addCallTime() async {
    ...
    var doc_res = await db
      ...
      toFirestore: (ChatCall msg, options) => msg.toFirestore(),
        ...
        .add(msgdata);
    // String sendcontent = "Voice call time: ${state.call_time_num.value}";
    String sendContent = "${state.call_time_num.value} " + " " + "📽️";
    // String sendContent = "${state.call_time_num.value}" + IconButton(
    //   onPressed: () {},
    //   icon: Icon(Icons.call_missed),
    // ).icon.toString();

    // print(sendContent);
    if (state.call_time.value == '00:00') {
      sendMessage(sendContent);
    } else {
      sendMessage("Video call time: ${state.call_time_num.value}");
    }
    // sendMessage(sendcontent);



    // String sendcontent = " Video call time: ${state.call_time_num.value}";
    // sendMessage(sendcontent);
  }
  ...
  turnCamera()async{
    await engine.enableLocalVideo(!state.turnCamera.value);
    state.turnCamera.value = !state.turnCamera.value;
  }
}
